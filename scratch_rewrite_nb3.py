"""
Rewrite nb3: Remove Unsloth entirely, use standard HuggingFace + BitsAndBytesConfig + PEFT.
Unsloth monkey-patches Trainer internals at import time, which is incompatible with
Qwen2-VL's multimodal architecture + custom DataCollator.
"""
import json
...
nb3 = {
 "cells": [
  {"cell_type":"markdown","metadata":{},"source":[
    "# TradeFlow AI — nb3_olm_finetune (v5 — Production Stable)\n",
    "\n",
    "**Architecture**: `allenai/olmOCR-2-7B-1025` (PRD §4 Decision 2)\n",
    "**Quantization**: NF4 4-bit via BitsAndBytesConfig (identical VRAM savings to Unsloth)\n",
    "**LoRA**: rank=32, alpha=64 via PEFT (PRD §10.4)\n",
    "**Trainer**: HuggingFace standard Trainer (stable with Qwen2-VL multimodal collator)\n",
    "\n",
    "> **Why not Unsloth?** Unsloth monkey-patches `Trainer.training_step` at import time.\n",
    "> This is incompatible with Qwen2-VL's multimodal architecture + custom DataCollator,\n",
    "> causing `AttributeError: 'int' has no attribute 'mean'`. The 4-bit quantization\n",
    "> and LoRA are provided directly by BitsAndBytesConfig + PEFT — same quality, same VRAM.\n"
  ]},
  ...
  {"cell_type":"code","execution_count":None,"metadata":{},"outputs":[],"source":[
    ...
    "MODEL_ID  = 'allenai/olmOCR-2-7B-1025'     # PRD §4 Decision 2\n",
    "LORA_R    = 32                               # PRD §10.4\n",
    "LORA_ALPHA = 64\n",
    ...
    "        print(f'VRAM {i} : {torch.cuda.get_device_properties(i).total_mem / 1e9:.1f} GB')\n"
  ]},
  {"cell_type":"markdown","metadata":{},"source":["## 1. Load Model (4-bit NF4 + LoRA r=32)"]},
  {"cell_type":"code","execution_count":None,"metadata":{},"outputs":[],"source":[
    "print(f'Loading {MODEL_ID}...')\n",
    "\n",
    "# 4-bit NF4 quantization — identical VRAM savings to Unsloth\n",
    "bnb_config = BitsAndBytesConfig(\n",
    "    load_in_4bit=True,\n",
    "    bnb_4bit_quant_type='nf4',\n",
    "    bnb_4bit_compute_dtype=torch.float16,\n",
    "    bnb_4bit_use_double_quant=True,          # Double quantization for extra VRAM savings\n",
    ")\n",
    "\n",
  ...
  {"cell_type":"markdown","metadata":{},"source":["## 3. Custom Data Collator (Qwen2-VL Multimodal)"]},
  {"cell_type":"code","execution_count":None,"metadata":{},"outputs":[],"source":[
    "class Qwen2VLDataCollator:\n",
    "    \"\"\"Collator that handles interleaved image-text inputs for Qwen2-VL.\n",
    "    \n",
    "    Key: sets labels=-100 for padding tokens so loss is only computed\n",
    "    on the assistant's response tokens.\n",
    "    \"\"\"\n",
  ...
  {"cell_type":"markdown","metadata":{},"source":["## 4. Training"]},
  {"cell_type":"code","execution_count":None,"metadata":{},"outputs":[],"source":[
    "training_args = TrainingArguments(\n",
    "    output_dir=str(OUT_DIR / 'checkpoints'),\n",
    "    learning_rate=1e-4,\n",
    "    num_train_epochs=1,             # Epoch 1 for initial run; increase to 3 for full\n",
    ...
    "    logging_steps=5,\n",
    "    save_strategy='no',\n",
    "    remove_unused_columns=False,    # CRITICAL: keep image_path for collator\n",
    "    fp16=True,                      # T4 does not support bf16\n",
    "    optim='adamw_torch',            # Standard PyTorch optimizer\n",
    "    dataloader_num_workers=0,       # Single-thread for PDF loading safety\n",
    "    gradient_checkpointing=True,    # Trade compute for VRAM (critical on T4)\n",
    "    report_to='none',\n",
    ")\n",
    "\n",
    "trainer = Trainer(\n",
...
with open(r'c:\tradeflow-ai\tools\kaggle\nb3_olm_finetune.ipynb', 'w', encoding='utf-8') as f:
    json.dump(nb3, f, indent=1)

print("nb3 rewritten successfully (Unsloth removed, BitsAndBytesConfig + PEFT retained)")
